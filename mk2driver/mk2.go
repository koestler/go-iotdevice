...
func NewMk2Connection(dev io.ReadWriter, logDebug bool) (Mk2, error) {
	mk2 := &mk2Ser{}
	mk2.p = dev
	mk2.info = &Mk2Info{}
	mk2.scaleCount = 0
...
func (m *mk2Ser) handleFrame(l byte, frame []byte) {
	if m.logDebug {
		log.Printf("mk2driver: frame %#v", frame)
	}
	if checkChecksum(l, frame[0], frame[1:]) {
		...
		if m.logDebug {
			log.Printf("mk2driver: Invalid incoming frame checksum: %x", frame)
		}
		m.frameLock = false
	}
}
...
func (m *mk2Ser) setTarget() {
	...
	cmd[2] = 0x00
	if m.logDebug {
		log.Printf("mk2driver: cmd setTargetFrame")
	}
	m.sendCommand(cmd)
}
...
func (m *mk2Ser) reqScaleFactor(in byte) {
	...
	cmd[2] = in
	if m.logDebug {
		log.Printf("mk2driver: cmd winmonFRame commandGetRAMVarInfo")
	}
	m.sendCommand(cmd)
}
...
func (m *mk2Ser) scaleDecode(frame []byte) {
	tmp := scaling{}
	if m.logDebug {
		log.Printf("mk2driver: Scale frame(%d): 0x%x", len(frame), frame)
	}
	if len(frame) < 6 {
		tmp.supported = false
		if m.logDebug {
			log.Printf("mk2driver: Skiping scaling factors for: %d", m.scaleCount)
		}
	} else {
		tmp.supported = true
		var scl int16
	...
	if m.logDebug {
		log.Printf("mk2driver: scalecount %v: %#v \n", m.scaleCount, tmp)
	}
	...
	if m.scaleCount < ramVarMaxOffset {
		...
		log.Printf("mk2driver: Monitoring starting")
	}
}
...
func (m *mk2Ser) versionDecode(frame []byte) {
	if m.logDebug {
		log.Printf("mk2driver: versiondecode %v", frame)
	}
	m.info.Version = 0
	m.info.Valid = true
	for i := 0; i < 4; i++ {
	...
	if m.scaleCount < ramVarMaxOffset {
		if m.logDebug {
			log.Printf("mk2driver: Get scaling factors")
		}
		m.reqScaleFactor(byte(m.scaleCount))
	} else {
		// set target to first device
		m.setTarget()
		...
		cmd[1] = infoReqAddrDC
		if m.logDebug {
			log.Printf("mk2driver: cmd infoReqFrame infoReqAddrDC")
		}
		m.sendCommand(cmd)
	}
}
...
func (m *mk2Ser) dcDecode(frame []byte) {
	...
	if m.logDebug {
		log.Printf("mk2driver: cmd infoReqFrame infoReqAddrACL1")
	}
	m.sendCommand(cmd)
}
...
func (m *mk2Ser) acDecode(frame []byte) {
	...
	if m.logDebug {
		log.Printf("mk2driver: acDecode %#v", m.info)
	}
	...
	cmd[0] = ledFrame
	if m.logDebug {
		log.Printf("mk2driver: cmd ledFrame")
	}
	m.sendCommand(cmd)
}
...
func (m *mk2Ser) stateDecode(frame []byte) {
	...
	if m.logDebug {
		log.Printf("mk2driver: battery state decode %#v", m.info)
	}
	m.updateReport()
}
...
func (m *mk2Ser) ledDecode(frame []byte) {
	...
	if m.logDebug {
		log.Printf("mk2driver: ledsOn: %#v, ledsBlink: %#x", frame[0], frame[1])
	}
	...
	if m.logDebug {
		log.Printf("mk2driver: cmd hwinmonFRame commandReadRAMVar ramVarChargeState")
	}
	m.sendCommand(cmd)
}
...
func (m *mk2Ser) sendCommand(data []byte) {
	...
	if m.logDebug {
		log.Printf("mk2driver: sendCommand %#v", dataOut)
	}
