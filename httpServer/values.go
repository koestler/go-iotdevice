	...
	for _, v := range env.Views {
		...
		for _, vd := range view.Devices() {
			...
			if env.Config.LogConfig() {
				log.Printf("httpServer: GET /api/v2/views/%s/devices/%s/values -> serve values as json", view.Name(), viewDevice.Name())
			}
		}
	}
}
...
func setupValuesPatch(mux *http.ServeMux, env *Environment) {
	...
	for _, v := range env.Views {
		...
		for _, dn := range view.Devices() {
			...
			if env.Config.LogConfig() {
				log.Printf("httpServer: PATCH /api/v2/views/%s/devices/%s/values -> setup value dispatcher", view.Name(), deviceName)
			}
		}
	}
}
