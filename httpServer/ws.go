...
func setupValuesWs(mux *http.ServeMux, env *Environment) {
	...
	for _, view := range env.Views {
		...
		logPrefix := fmt.Sprintf("httpServer: /api/v2/views/%s/ws", view.Name())
				...
				if mt == websocket.MessageText {
					...
					if err := json.Unmarshal(msg, &authMsg); err == nil {
						if user, err := checkToken(authMsg.AuthToken, env.Authentication.JwtSecret()); err == nil {
							log.Printf("httpServer: /api/v2/views/%s/ws: user=%s authenticated", view.Name(), user)
		...
		if env.Config.LogConfig() {
			log.Printf("httpServer: GET /api/v2/views/%s/ws -> setup websocket for view", view.Name())
		}
	}
}
