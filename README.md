...
MqttClients:                                               # optional, when empty, no mqtt connection is made
  local:                                                   # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    ConnectRetryDelay: 10s                                 # optional, default 10s, when disconnected: delay before a connection attempt is made
    ConnectTimeout: 5s                                     # optional, default 5s, how long to wait for the SYN+ACK packet, increase on slow networks
    TopicPrefix: go-iotdevice/                             # optional, default go-iotdevice/, %Prefix% is replaced with this string
    ReadOnly: false                                        # optional, default false, when true, no messages are sent to the server (overrides all section Enabled flags and MaxBacklogSize)
    ...
    AvailabilityClient:
      ...
      TopicTemplate: '%Prefix%avail/%ClientId%'            # optional, default as shown, what topic to use for online/offline messages of the go-iotdevice instance
    ...
    AvailabilityDevice:
      Enabled: true                                        # optional, default true, whether to send online messages and register an offline message as will
      TopicTemplate: '%Prefix%avail/%DeviceName%'          # optional, default as shown, what topic to use for online/offline messages of a specific device
      Retain: true                                         # optional, default true, the mqtt retain flag for availability messages
      Qos: 1                                               # optional, default 1, what quality-of-service level shall be used
    ...
    Structure:
      ...
      TopicTemplate: '%Prefix%struct/%DeviceName%'         # optional, default as shown, what topic to use for structure messages
    ...
    Telemetry:
      ...
      TopicTemplate: '%Prefix%tele/%DeviceName%'           # optional, default as shown, what topic to use for telemetry messages
      Interval: 1s                                         # optional, default 1s, how often to send telemetry mqtt messages
    ...
    Realtime:
      ...
      TopicTemplate: '%Prefix%real/%DeviceName%/%RegisterName%' # optional, default as shown, what topic to use for real-time messages
    ...
    HomeassistantDiscovery:
      Enabled: true                                        # optional, default false, whether to enable sending homeassistant auto-discovery messages
      TopicTemplate: 'homeassistant/%Component%/%NodeId%/%ObjectId%/config' # optional, default as shown, topic to use for homeassistant discovery messages
      Interval: 0s                                         # optional, default 0, 0 means only send initially, otherwise the discovery messages are repeated after this interval (useful when retain is false)
      Retain: false                                        # optional, default false, the mqtt retain flag for homeassistant discovery messages
      Qos: 1                                               # optional, default 1, what quality-of-service level shall be used
      Devices:                                             # optional, default all, a list of devices to match
        bmv0:                                              # use device identifiers of the VictronDevices, ModbusDevices, etc. sections
          Filter:                                          # optional, default include all, defines which registers are shown in the view,
    ...
    Command:
      ...
      TopicTemplate: '%Prefix%cmnd/%DeviceName%/%RegisterName%' # optional, default as shown, what topic to use for command messages
      Qos: 1                                               # optional, default 1, what quality-of-service level shall be used
      Devices:                                             # optional, default all, a list of devices to match
        bmv0:                                              # messages are only sent for this device
          Filter:                                          # optional, default include all, defines which registers are shown in the view,
...
VictronDevices:                                            # optional, a list of Victron Energy devices to connect to
  bmv0:                                                    # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    Kind: Vedirect                                         # mandatory, possibilities: Vedirect, RandomBmv, RandomSolar, always set to Vedirect except for development
...
ModbusDevices:                                             # optional, a list of devices connected via ModBus
  modbus-rtu0:                                             # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    Relays:                                                # optional, default empty, a map of custom labels for the relays
      CH1:
        Description: Lamp                                  # optional, default name, a nice title displayed in the frontend
  ...
  modbus-finder:                                           # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    Address: 33                                            # mandatory, the modbus address of the device, either decimal (e.g. 33) or hex string (e.g. 0x0A)
...
GpioDevices:                                               # optional, a list of devices controlled via gpio
  gpio0:                                                   # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    Outputs:                                               # optional, a list of outputs
      Relay0:                                              # mandatory, a technical name used for the register
      ...
      Relay1:                                              # mandatory, a technical name used for the register
...
HttpDevices:                                               # optional, a list of devices controlled via http
  tcw241:                                                  # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    Username: admin                                        # optional, default empty, username used to log in
    Password: my-secret                                    # optional, default empty, password used to log in
    PollInterval: 1s                                       # optional, default 1s, how often to fetch the device status
    Filter:                                                # optional, default include all, defines which registers are shown in the view,
      # The rules are applied in order beginning with IncludeRegisters (highest priority) and ending with DefaultInclude (lowest priority).
      IncludeRegisters:                                    # optional, default empty, if a register is on this list, it is returned
...
GensetDevices:                                             # optional, a list of generator set control devices
  genset0:                                                 # mandatory, an arbitrary name used for logging and for referencing in other config sections
    ...
    EngineTempMin: -20                                     # optional, default -20, minimum temperature the engine must have to not trigger the error state
...
Views:                                                     # optional, a list of views (=categories in the frontend / paths in the api URLs)
  - Name: victron                                          # mandatory, a technical name used in the URLs
    ...
    Devices:                                               # mandatory, a list of devices to show
